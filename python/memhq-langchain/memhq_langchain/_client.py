"""Tiny shared HTTP layer for the LangChain adapter.

We hand-roll a minimal httpx client instead of taking a hard dep on the
``memhq`` package — keeps the adapter installable standalone for users
who only want the LangChain surface. The shape mirrors the official SDK
...
USER_AGENT = "memhq-langchain-python/0.1.0"
...
class _Transport:
    """Shared sync + async HTTP transport."""
...
async def aadd_messages(
    ...
    *,
    ...
    if metadata:
        body["metadata"] = metadata
    return await transport.apost("/v1/memhq/add", body)
